...
server.registerTool(
    'now_time',
    {
        ...
        inputSchema: z.object({
            country: z.string().describe('현재 시간을 조회할 나라 이름 (예: Korea, Japan, USA, France)')
        }),
        outputSchema: z.object({
            content: z
                .array(
    ...
    async ({ country }) => {
        ...
        if (!timezone) {
            const errorText = `'${country}'에 해당하는 시간대를 찾을 수 없습니다. 다른 나라 이름으로 시도해 보세요.`
            return {
                content: [{ type: 'text' as const, text: errorText }],
                structuredContent: {
                    content: [{ type: 'text' as const, text: errorText }]
        ...
        const text = `${country}의 현재 시간: ${formatter.format(now)} (${timezone})`
        return {
            content: [{ type: 'text' as const, text }],
            structuredContent: {
                content: [{ type: 'text' as const, text }]
...
server.registerTool(
    'geocode',
    {
        description: '도시 이름이나 주소를 입력하면 Nominatim OpenStreetMap API를 통해 위도와 경도 좌표를 반환합니다.',
        inputSchema: z.object({
            query: z.string().describe('좌표를 조회할 도시 이름 또는 주소 (예: 서울, Tokyo, Paris, 1600 Pennsylvania Ave Washington DC)'),
            limit: z
                .number()
                .int()
                .min(1)
    ...
    async ({ query, limit }) => {
        ...
        if (!response.ok) {
            const errorText = `Nominatim API 오류: HTTP ${response.status} ${response.statusText}`
            return {
                content: [{ type: 'text' as const, text: errorText }],
                structuredContent: {
                    content: [{ type: 'text' as const, text: errorText }]
        ...
        if (results.length === 0) {
            const errorText = `'${query}'에 대한 검색 결과를 찾을 수 없습니다.`
            return {
                content: [{ type: 'text' as const, text: errorText }],
                structuredContent: {
                    content: [{ type: 'text' as const, text: errorText }]
        ...
        const text = `"${query}" 검색 결과 (${results.length}건):\n\n${lines.join('\n\n')}`
        return {
            content: [{ type: 'text' as const, text }],
            structuredContent: {
                content: [{ type: 'text' as const, text }]
...
server.registerTool(
    'get-weather',
    {
        description: '위도, 경도, 예보 기간을 입력하면 Open-Meteo API를 통해 현재 날씨와 일별 예보를 반환합니다.',
        inputSchema: z.object({
            latitude: z.number().min(-90).max(90).describe('위도 (예: 37.5665 = 서울)'),
            longitude: z.number().min(-180).max(180).describe('경도 (예: 126.9780 = 서울)'),
            forecast_days: z
                .number()
                .int()
                .min(1)
    ...
    async ({ latitude, longitude, forecast_days }) => {
        ...
        if (!response.ok) {
            ...
            const errorText = `Open-Meteo API 오류: ${body.reason ?? `HTTP ${response.status}`}`
            return {
                content: [{ type: 'text' as const, text: errorText }],
...
server.registerTool(
    'generate-image',
    {
        description: '텍스트 프롬프트를 입력하면 HuggingFace FLUX.1-schnell 모델로 이미지를 생성해 반환합니다.',
        inputSchema: z.object({
            ...
            num_inference_steps: z
                ...
                .optional()
                .default(4)
                .describe('추론 스텝 수 (기본값: 4, 범위: 1~10). 높을수록 품질이 좋아지지만 느려집니다.')
        })
    },
...
server.registerResource(
    ...
    {
        title: '서버 상태',
        description: 'toni-mcp-server의 실시간 가동 시간, 메모리 사용량, 현재 시각을 반환합니다.',
        mimeType: 'application/json'
    },
    async (uri) => {
        const now = new Date()
...
server.registerPrompt(
    ...
    {
        title: '코드 리뷰',
        description: '코드를 입력하면 품질, 가독성, 보안, 성능 관점에서 전문적인 코드 리뷰를 제공합니다.',
        argsSchema: {
            ...
            focus: z
                ...
                .optional()
                .default('all')
                .describe('리뷰 집중 영역: all(전체), quality(코드 품질), security(보안), performance(성능), readability(가독성)')
        }
    },
    ({ code, language, focus }) => {
